...
#Preview("Design System Components") {
    VStack(spacing: 24) {
        HStack(spacing: 16) {
            DSPrimaryButton("Primary", icon: "arrow.right") {}
            DSSecondaryButton("Secondary", icon: "gear") {}
        }

        HStack(spacing: 8) {
            DSIconButton(icon: "crop", isSelected: true) {}
            DSIconButton(icon: "rectangle") {}
            DSIconButton(icon: "arrow.right") {}
            DSIconButton(icon: "pencil") {}
        }

        HStack(spacing: 8) {
            DSChip("All", isSelected: true) {}
            DSChip("Screenshots", icon: "camera") {}
            DSChip("Recordings", icon: "video") {}
        }

        HStack(spacing: 8) {
            DSActionCard(icon: "doc.on.clipboard", title: "Copy", shortcut: "⌘C") {}
            DSActionCard(icon: "square.and.arrow.down", title: "Save", shortcut: "⌘S") {}
            DSActionCard(icon: "pencil", title: "Edit", shortcut: "⌘E") {}
        }

        HStack(spacing: 4) {
            ForEach([Color.red, .orange, .yellow, .green, .blue, .purple], id: \.self) { color in
                DSColorSwatch(color: color, isSelected: color == .red) {}
            }
        }
    }
    .padding(32)
